public class PlayingState implements State {
    public static final PlayingState INSTANCE = new PlayingState();
    private PlayingState() { }

    @Override
    public void play(MediaPlayer player) {
    }

    @Override
    public void pause(MediaPlayer player) {
        System.out.println("Paused track: " + player.getCurrentTrack());
        player.setState(PausedState.INSTANCE);
    }

    @Override
    public void next(MediaPlayer player) {
        int nextIndex = player.getCurrentTrackNum() + 1;
        if (nextIndex < player.getTracks().size()) {
            player.setTrackNum(nextIndex);
            System.out.println("Next track: " + player.getCurrentTrack());
        }
    }

    @Override
    public void prev(MediaPlayer player) {
        int prevIndex = player.getCurrentTrackNum() - 1;
        if (prevIndex >= 0) {
            player.setTrackNum(prevIndex);
            System.out.println("Previous track: " + player.getCurrentTrack());
        }
    }

    @Override
    public void stop(MediaPlayer player) {
        System.out.println("Stopped");
        player.setState(StoppedState.INSTANCE);
    }
}
